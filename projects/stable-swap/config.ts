//Config
export default {
  A: {
	//For - Sei
    seiMainnet: 1000,
    seiTestnet: 1000,
	//For - Monad
	monadMainnet: 1000,
    monadTestnet: 1000,
	//For - Tics
	ticsMainnet: 1000,
    ticsTestnet: 1000,
	//For - Bsc
	bscMainnet: 1000,
    bscTestnet: 1000,
	//For - Avax
	avaxMainnet: 1000,
    avaxTestnet: 1000,
  },
  Fee: {
	//For - Sei  
    seiMainnet: 4000000,
    seiTestnet: 4000000,
	//For - Monad
    monadMainnet: 4000000,
    monadTestnet: 4000000,
	//For - Tics
    ticsMainnet: 4000000,
    ticsTestnet: 4000000,
	//For - Bsc
    bscMainnet: 4000000,
    bscTestnet: 4000000,
	//For - Avax
    avaxMainnet: 4000000,
    avaxTestnet: 4000000,
  },
  AdminFee: {
	//For - Sei  
    seiMainnet: 5000000000,
    seiTestnet: 5000000000,
	//For - Monad
    monadMainnet: 5000000000,
    monadTestnet: 5000000000,
	//For - Tics
    ticsMainnet: 5000000000,
    ticsTestnet: 5000000000,
	//For - Bsc
    bscMainnet: 5000000000,
    bscTestnet: 5000000000,
	//For - Avax
    avaxMainnet: 5000000000,
    avaxTestnet: 5000000000,
  },
  LPFactory: {
	//For - Sei  
    seiMainnet: "0x614f1436e6b9b686781428c4F17a0CF425E3ce7d", //official
	//For - Monad
    monadMainnet: "0x16fb89911d7B87daB56510dCB91C2027f9B020b9", //official
	//For - Tics
    ticsMainnet: "0xCFC6A070404c068f21Bc675B03F7c45331b7Fbe1", //official
	//For - Bsc
    bscMainnet: "0x", //official
	//For - Avax
    avaxMainnet: "0x", //official
  },
  SwapTwoPoolDeployer: {
	//For - Sei  
    seiMainnet: "0xC911C401b7d1e896Bed83d7D42fBBC8A55A33F22", //official
	//For - Monad 
    monadMainnet: "0x83F77ec45AECd6AaA975D43d7710c5349A078B71", //official
	//For - Tics
    ticsMainnet: "0x6BFC02F34d09846dfE2555eAEB61fe7c5dd700F6", //official
	//For - Bsc
    bscMainnet: "0x", //official
	//For - Avax
    avaxMainnet: "0x", //official
  },
  SwapThreePoolDeployer: {
	//For - Sei  
    seiMainnet: "0xd4c54a77bE8a4107445B69ac653C5874761314d3", //official
	//For - Monad
    monadMainnet: "0xae5095cF0999FeAeF62A07bF0465f4835944D697", //official
	//For - Tics
    ticsMainnet: "0x16fb89911d7B87daB56510dCB91C2027f9B020b9", //official
	//For - Bsc
    bscMainnet: "0x", //official
	//For - Avax
    avaxMainnet: "0x", //official
  },
  Factory: { //qubeStableSwapFactory - stableFactory
    //For - Sei  
    seiMainnet: "0xbbD7e8B7949300f9C36BcDA83Ee4589FF955BE50", //official
	//For - Monad  
    monadMainnet: "0x931765421E5970dda3e3499449A67fb03f4ebD4C", //official
	//For - Tics  
    ticsMainnet: "0x83F77ec45AECd6AaA975D43d7710c5349A078B71", //official
	//For - Bsc
    bscMainnet: "0x", //official
	//For - Avax
    avaxMainnet: "0x", //official
  },
  TwoPoolInfo: {
	//For - Sei  
    seiMainnet: "0xeeA08e2085b0cf11135ee232024ad6131ccd5fBb", //official
	//For - Monad  
    monadMainnet: "0x11A51A136DE68347c9e2e76A2B1F517CE43cfc35", //official
	//For - Tics  
    ticsMainnet: "0xae5095cF0999FeAeF62A07bF0465f4835944D697", //official
	//For - Bsc
    bscMainnet: "0x", //official
	//For - Avax
    avaxMainnet: "0x", //official
  },
  ThreePoolInfo: {
	//For - Sei  
    seiMainnet: "0xF6B2DA8EEFC9BadA0C52740B2a444Fde448EeA2d", //official
	//For - Monad 
    monadMainnet: "0xb38dC4991350a0145b37347afC988c99C5fac036", //official
	//For - Tics
    ticsMainnet: "0x931765421E5970dda3e3499449A67fb03f4ebD4C", //official
	//For - Bsc
    bscMainnet: "0x", //official
	//For - Avax
    avaxMainnet: "0x", //official
  },
  Info: { //qubeStableSwapInfo - stableInfo
    //For - Sei  
    seiMainnet: "0x187246fF29915f9733c961d288af0E6094b9e2FA", //official
	//For - Monad
    monadMainnet: "0x38Ec4aDBabfEb321D512769734e3731Ccf3a8AFf", //official
	//For - Tics
    ticsMainnet: "0x11A51A136DE68347c9e2e76A2B1F517CE43cfc35", //official
	//For - Bsc
    bscMainnet: "0x", //official
	//For - Avax
    avaxMainnet: "0x", //official
  },
  WBNB: {
	// WSEI  
    seiMainnet: "0xE30feDd158A2e3b13e9badaeABaFc5516e95e8C7",
	// WMON
    monadMainnet: "0x3bd359C1119dA7Da1D913D1C4D2B7c461115433A",
	// WTICS
    ticsMainnet: "0x8A0a018caEd522B3f1501616f828f6B1819Ced22",
	// WBNB
    bscMainnet: "0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c",
	// WAVAX
    avaxMainnet: "0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7",
  },
};
